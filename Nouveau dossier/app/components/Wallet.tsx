...
const Wallet: React.FC<WalletProps> = ({ credits, transactions, onAddCredits, userSettings, onUpdateUser }) => {
  ...
  return (
        <div className="max-w-3xl mx-auto space-y-8 relative pb-20">
            ...
            {showSuccess && (
                <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] bg-green-500 text-white px-6 py-3 rounded-xl shadow-lg flex items-center gap-2 animate-fade-in">
                ...
                >
                    ...
                    <div className="flex justify-between items-start mb-2">
                         <div className={`p-3 rounded-full ${pkg.popular ? 'bg-yellow-100' : 'bg-slate-100 group-hover:bg-blue-50'}`}>
                            <Coins className={`w-6 h-6 ${pkg.popular ? 'text-yellow-600 fill-yellow-600' : 'text-slate-500 group-hover:text-black'}`} />
                         </div>
                    </div>
                    ...
                    <div key={tx.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                        <div className="flex items-center gap-4">
                            <div className={`p-2 rounded-full ${
                                ...
                                tx.type === 'purchase' ? 'bg-blue-100 text-black' : 
      ...
      {showAdModal && (
           <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black animate-fade-in">
               <div className="relative w-full h-full flex flex-col items-center justify-center p-4">
                   ...
                   {/* Google AdSense Video Ad Placeholder */}
                   <div className="w-full max-w-lg aspect-video bg-white rounded-2xl border border-slate-300 flex items-center justify-center relative overflow-hidden shadow-2xl mb-4">
                       {/* AdSense script and ad unit */}
                       <div style={{ width: '100%', height: '100%' }}>
                         {/* AdSense script tag */}
                         <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-2474444884447314" crossOrigin="anonymous"></script>
                         {/* AdSense ad unit placeholder (replace with your actual ad unit code if needed) */}
