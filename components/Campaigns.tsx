...
import { Campaign, Platform, TaskType, CampaignStatus, AiAdvice, COUNTRIES_LIST } from '../types';
import { analyzeCampaignContent } from '../services/gemini';
import { Plus, Sparkles, Target, BarChart2, Check, AlertCircle, Settings, Globe, Smartphone, MapPin, Search, ChevronDown, X, MoreVertical, PauseCircle, PlayCircle, Trash2, Edit2, Coins } from 'lucide-react';
...
const Campaigns: React.FC<CampaignsProps> = ({ 
  ...
  const [reward, setReward] = useState(2); // New Reward State
  ...
  const totalCost = count * costPerAction;
  const canAfford = userCredits >= totalCost;

  const handleAnalyze = async () => {
    if (!description && !url) return;
    setIsAnalyzing(true);
    const context = `URL: ${url} | Description: ${description || "No description provided."}`;
    const advice = await analyzeCampaignContent(context);
    setAiAdvice(advice);
    setIsAnalyzing(false);
    
    if (advice.suggestedPlatform && Object.values(Platform).includes(advice.suggestedPlatform as Platform)) {
        setPlatform(advice.suggestedPlatform as Platform);
    }
  };
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        {view === 'list' && (
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl font-medium flex items-center shadow-lg shadow-indigo-200 transition-all active:scale-95"
          >
      ...
      {view === 'create' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div>
                   ...
                   <div className="relative">
                    ...
                    <button 
                        ...
                        className="absolute bottom-3 ltr:right-3 rtl:left-3 bg-gradient-to-r from-indigo-500 to-violet-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg flex items-center shadow-sm hover:opacity-90 transition-opacity disabled:opacity-50"
                    >
                        {isAnalyzing ? (
                            <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin ltr:mr-2 rtl:ml-2" />
                        ) : (
                            <Sparkles className="w-3 h-3 ltr:mr-1.5 rtl:ml-1.5" />
                        )}
                        {t('camp.ai_optimize')}
                    </button>
                   </div>
                ...
                {/* Quantity Input */}
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">{t('camp.quantity')}</label>
                    <input 
                        type="number" 
                        min="5" 
                        value={count} 
                        onChange={(e) => setCount(Math.max(5, parseInt(e.target.value) || 0))}
          ...
          <div className="space-y-6">
            {/* AI Insights Card */}
            {aiAdvice && (
                <div className="bg-gradient-to-br from-violet-50 to-indigo-50 border border-indigo-100 p-5 rounded-2xl animate-fade-in">
                    <h3 className="text-indigo-900 font-bold flex items-center mb-3">
                        <Sparkles className="w-4 h-4 ltr:mr-2 rtl:ml-2 text-indigo-600" /> AI Strategy
                    </h3>
                    <div className="space-y-3 text-sm">
                        <div className="flex items-start">
                            <Target className="w-4 h-4 ltr:mr-2 rtl:ml-2 text-indigo-500 mt-0.5" />
                            <div>
                                <span className="font-semibold text-indigo-900">Target:</span> 
                                <p className="text-indigo-800/80 leading-snug">{aiAdvice.targetAudience}</p>
                            </div>
                        </div>
                        <div className="flex items-center">
                            <BarChart2 className="w-4 h-4 ltr:mr-2 rtl:ml-2 text-indigo-500" />
                            <span className="font-semibold text-indigo-900 ltr:mr-2 rtl:ml-2">Virality Score:</span>
                            <div className="flex-1 h-2 bg-indigo-200 rounded-full overflow-hidden">
                                <div className="h-full bg-indigo-500" style={{ width: `${aiAdvice.viralityScore * 10}%` }}></div>
                            </div>
                            <span className="ltr:ml-2 rtl:mr-2 text-xs font-bold text-indigo-700">{aiAdvice.viralityScore}/10</span>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm opacity-80 hover:opacity-100 transition-opacity">
                ...
                <div className="flex items-center gap-3">
                     ...
                     <div>
                        ...
                        <div className="text-xs text-slate-500">{platform} • +{reward} Coins</div>
                     </div>
                     <button className="ltr:ml-auto rtl:mr-auto bg-indigo-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg">
                        Do Task
                     </button>
                </div>
            </div>
            ...
            <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl">
                ...
                <div className="space-y-3 mb-6 text-sm text-slate-300">
                    ...
                    <div className="flex justify-between">
                        ...
                        <span className="text-white font-medium">{targeting.country}</span>
                    </div>
                    <div className="flex justify-between">
                        <span>Coins per Action</span>
                        <span className="text-yellow-400 font-bold">{reward} coins</span>
                    </div>
                    <div className="flex justify-between pt-3 border-t border-slate-700">
                        <span className="font-medium text-white">{t('camp.total_cost')}</span>
                        <span className="font-bold text-xl text-yellow-400">{totalCost} coins</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <button
                        ...
                        disabled={!canAfford || !url}
                        className="px-4 py-3 rounded-xl font-bold bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-lg shadow-indigo-900/20"
                    >
                        {t('camp.launch')}
                    </button>
                </div>
        ...
        <div className="space-y-4">
          {campaigns.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
                ...
                <button
                    onClick={() => setView('create')}
                    className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-medium shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-colors"
                >
                    {t('camp.new')}
                </button>
            </div>
                     ...
                     <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end flex-1 pl-4">
                        <div className="flex flex-col items-end flex-1">
                            ...
                            <div className="w-full sm:w-48 h-2 bg-slate-100 rounded-full overflow-hidden">
                                <div 
                                    className={`h-full rounded-full transition-all duration-1000 ${campaign.status === 'Paused' ? 'bg-slate-400' : 'bg-indigo-600'}`}
